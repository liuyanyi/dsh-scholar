...
export const NativeGpuDevice = z.object({ index: z.string().regex(/^\d+$/), uuid: z.string().regex(/^GPU-[a-fA-F0-9-]+$/), name: z.string().max(160).optional() }).strict()
...
/** Versioned software identity; the original snapshot/hash remains a full observation. */
export function nativeSoftwareEnvironmentHash(value: NativeEnvironmentSnapshot): string {
  const { gpu_uuids: _devices, schema_version: _version, ...software } = NativeEnvironmentSnapshot.parse(value)
  return `sha256:${createHash('sha256').update(canonicalJsonDeep({ schema_version: 2, ...software })).digest('hex')}`
}
...
export const ContainerNativeFingerprint = z.object({
  ...
  gpu_devices: z.array(NativeGpuDevice)
    .refine(devices => new Set(devices.map(d => d.index)).size === devices.length && new Set(devices.map(d => d.uuid)).size === devices.length, 'GPU observations must be unique'),
  dependency_lock_hash: z.string().regex(/^sha256:[a-f0-9]{64}$/).nullable(),
  actual_environment_hash: sha256.optional(),
  software_environment_hash: sha256.optional(),
  selected_gpu_uuids: z.array(z.string().regex(/^GPU-[a-fA-F0-9-]+$/)).optional(),
  requested_gpu_devices: z.array(NativeGpuDevice).optional(),
  ...
  .refine(value => value.schema_version !== 3 || (value.actual_environment_hash !== undefined && value.software_environment_hash !== undefined), 'v3 requires full and software environment identities')
